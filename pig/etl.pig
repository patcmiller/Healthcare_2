...
-- detect the events of alive patients and create it of the form (patientid, eventid, value, label, time_difference) where
-- time_difference is the days between index date and each event timestamp
a_idx= JOIN events BY patientid LEFT OUTER, mortality BY patientid;
a_idx= FILTER a_idx BY mtimestamp IS null;
m_idx= GROUP a_idx BY events::patientid; 
m_idx= FOREACH m_idx GENERATE group AS patientid, MAX(a_idx.etimestamp) AS idx;
aliveevents= JOIN m_idx BY patientid, a_idx BY events::patientid;
aliveevents= FOREACH aliveevents GENERATE m_idx::patientid, eventid, a_idx::events::value, 0, DaysBetween(idx,etimestamp) AS time_difference;
...
peid= GROUP filtered BY (patientid, eventid);
peid= FILTER peid BY filtered.value IS NOT null;
featureswithid= FOREACH peid GENERATE group.$0 AS patientid, group.$1 AS eventid, COUNT(filtered.value) AS featurevalue;
...
deid= FOREACH featureswithid GENERATE eventid;
deid= DISTINCT deid;
deid= RANK deid BY eventid;
all_features= FOREACH deid GENERATE rank_deid -1 as idx, eventid;
...
g_idx= GROUP features BY idx;
maxvalues= FOREACH g_idx GENERATE group AS idx, MAX(features.featurevalue) AS maxvalues; 
...
features= FOREACH normalized GENERATE patientid, maxvalues::idx AS idx, (double)featurevalue/(double)maxvalues AS normalizedfeaturevalue;  
...
doa= GROUP filtered BY patientid;
labels= FOREACH doa GENERATE group AS patientid, MAX(filtered.label) AS label;
