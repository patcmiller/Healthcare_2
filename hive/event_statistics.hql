...
-- ***** your code below *****
FROM events WHERE events.patient_id NOT IN (SELECT mortality.patient_id FROM mortality);
...
-- ***** your code below *****
FROM (SELECT count(alive_events.event_id) AS event_count FROM alive_events GROUP BY alive_events.patient_id) aeve_count;

-- dead patients
SELECT avg(event_count), min(event_count), max(event_count)
-- ***** your code below *****
FROM (SELECT count(dead_events.event_id) AS event_count FROM dead_events GROUP BY dead_events.patient_id) deve_count;
...
FROM (SELECT count(DISTINCT alive_events.time) AS encounter_count 
	  FROM alive_events GROUP BY alive_events.patient_id) aenc_count;	

-- dead
SELECT avg(encounter_count), min(encounter_count), max(encounter_count)
-- ***** your code below *****
FROM (SELECT count(DISTINCT dead_events.time) AS encounter_count 
	  FROM dead_events GROUP BY dead_events.patient_id) denc_count;
...
-- ***** your code below *****
FROM (SELECT datediff(hi_time, lo_time) AS record_length FROM (SELECT max(alive_events.time) AS hi_time, min(alive_events.time) AS lo_time
	  FROM alive_events GROUP BY alive_events.patient_id) a_time) arec_length;

-- dead
SELECT avg(record_length), min(record_length), max(record_length)
-- ***** your code below *****
FROM (SELECT datediff(hi_time, lo_time) AS record_length FROM (SELECT max(dead_events.time) AS hi_time, min(dead_events.time) AS lo_time
	  FROM dead_events GROUP BY dead_events.patient_id) d_time) drec_length;
